...
dialog={
	...
	alertChat:function(){//msgTitle：提示标题,msg：提示内容,func：为“确定”按钮动作处理函数,flag：双按钮标识，”d“为双按钮（”确定“和”取消“,“取消”时的按钮动作是关闭窗口），”s“为单按钮（只有”确定“）
		dialog.showInfo("<div class='chatWrap'>"
			+" <div class='p'><textarea class='chatArea'></textarea></div>"
	...
	alertLog:function(logTitle,func){//func：为“登录”按钮动作处理函数
		dialog.showInfo("<div class='logInfo'>"
			...
			+"  </div></li>"
			+"  <li class='yzM'><label for='yzM'>验证码：</label><input type='text' validate='true' name='yzM' id='yzM' maxlength='4' msg='请输入验证码'><img src='http://hd.wows.kongzhong.com/verifyCode?Math.random()' id='verifyImg' onclick='Util.verifyImg()'></li>"
			+"  <li class='tsli' id='logTs' style='display:none'><label for=''></label><span id='loginTips'></span></li>"
			+" </ul>"
